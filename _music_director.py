"""Phase 151: P1 'exploration/tension music katmanlari'. Recompile
(wire_playable_slice now also attaches Gameplay.AudioManager engine +
Gameplay.MusicDirector brain), reopen ForgottenValley_VS, wire the
slice, confirm both landed + missing_types empty, screenshot.
Daemon sleeps ~22 min between cycles (safe window).
"""
import time, json
...
print("WIRE:", json.dumps(res, ensure_ascii=False)[:760])

for nm in ("TI_Audio", "TI_MusicDirector"):
    d = rcall("get_object_details", {"name": nm}, t=20)
    comps = d.get("components") if isinstance(d, dict) else None
    print(f"{nm} comps:", json.dumps(comps, ensure_ascii=False)[:200])
...
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_music_director"))[:120])
print("MUSIC-DIRECTOR DONE ok=%s wired=%s missing=%s" % (
