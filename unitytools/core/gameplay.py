...
NEEDS_SCRIPT: frozenset[str] = frozenset({
    ...
    "pushable", "puzzle", "holdzone", "escort", "boss", "collectrace", "turret", "hitflash",
    "deadline", "lockgoal", "safezone", "wrapmover",
})
...
_ALIASES = {
    ...
    # escort: a VIP that walks itself to the goal (NOT "koru"/"koruma" -- those are guard terms)
    "escort": "escort", "eskort": "escort", "refakat": "escort", "vip": "escort",
    # boss: a single high-HP enemy for a duel
    "boss": "boss", "patron": "boss",
    # collectrace: the collector-race win/lose manager (collect all before the deadline)
    "collectrace": "collectrace", "collectorrace": "collectrace", "yaris": "collectrace",
    # turret: a stationary ranged threat that shoots the player (NOT "kule" -- tower_defense)
    "turret": "turret", "taret": "turret",
    # hitflash: visual hit feedback (renderer flashes on TakeDamage)
    "hitflash": "hitflash", "parlama": "hitflash", "flash": "hitflash",
    # deadline: a LOSING countdown (the mirror of timer). NOT "sure"/"zaman" -- those are
    # timer (a WINNING countdown); these are timeout/deadline-specific so they can't shadow it.
    "deadline": "deadline", "zamanasimi": "deadline", "sonsure": "deadline", "songerisayim": "deadline",
    # lockgoal: a locked exit that opens once every Key_* is collected (key-and-door win zone)
    "lockgoal": "lockgoal", "kilitkapi": "lockgoal", "kilitlikapi": "lockgoal",
    "lockeddoor": "lockgoal", "lockedexit": "lockgoal", "kilitlicikis": "lockgoal",
    # safezone: a retreat the chasers cannot enter (NOT "bolge" -- that is holdzone)
    "safezone": "safezone", "safe": "safezone", "guvenli": "safezone",
    "guvenlibolge": "safezone", "siginak": "safezone", "safehaven": "safezone",
    # wrapmover: wrapping lane traffic for a crossing (Frogger) game
    "wrapmover": "wrapmover", "lanemover": "wrapmover", "trafik": "wrapmover", "seritmover": "wrapmover",
}
...
_WRAPMOVER_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. Lane traffic for a crossing (Frogger-style) game:
// glides along +X at `speed` and WRAPS -- once it passes +bound it reappears at -bound, so each
// lane is an endless stream you must time your crossing through. Pair it with `killzone` (touch ->
// respawn). Deterministic: only position + Time, no RNG.
public class __CLASS__ : MonoBehaviour
{
...
// Auto-generated by UnityTools autopilot. Collectible pickup (trigger). When the Player gets
// within `magnetRange` the pickup gently DRIFTS toward them (a satisfying magnetic pull); on
// pickup it scores IMMEDIATELY (decoupled SendMessage) then plays a short scale-up "pop" before
// destroying itself -- juice on the collect moment so a grab feels good. Deterministic (only
// positions + Time, no RNG); the `collected` guard fires the score exactly once even during the
// pop, and the magnet is a no-op once collected (or with no Player in range).
...
public class __CLASS__ : MonoBehaviour
{
    ...
    void Update()
    {
        if (!collected)
        {
            // magnet: drift toward the Player when it is close (a gentle pull, no RNG). Only a
            // short-range assist -- you still have to come get it. No-op with no Player / too far.
            GameObject player = GameObject.FindWithTag("Player");
            if (player != null)
            {
                Vector3 d = player.transform.position - transform.position;
                float dist = d.magnitude;
                if (dist > 0.01f && dist < magnetRange)
                {
                    transform.position += d.normalized * magnetSpeed * Time.deltaTime;
                }
            }
            return;
        }
        ...
        transform.localScale = baseScale * (1f + (popScale - 1f) * t);
    }
}
"""
...
// it grants xpReward to the Player (SendMessage "AddXP"), plays a short scale-up
// "death pop" (Time-driven, the kill-moment cousin of the collectible pickup pop),
// then destroys itself. Fully decoupled: no reference to AutopilotXP or the
// attacker's type. Deterministic: only Time, no RNG.
public class __CLASS__ : MonoBehaviour
{
    public int maxHP = 30;
    ...
    public void TakeDamage(int amount)
    {
        if (dying) return;                                   // already dead -- ignore further hits
        currentHP -= amount;
        if (currentHP <= 0)
        {
            dying = true;
    ...
    void Update()
    {
        ...
        float t = popTime > 0f ? Mathf.Clamp01(dieElapsed / popTime) : 1f;
        transform.localScale = baseScale * (1f + (popScale - 1f) * t);   // a quick death pop
        if (dieElapsed >= popTime)
        {
            Destroy(gameObject);
        }
    }
...
_DEADLINE_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. A LOSING countdown -- the mirror of AutopilotTimer.
// Counts `duration` seconds down (Time.deltaTime, so it freezes while the game is paused on
// the title screen), draws the remaining time top-right, and on reaching zero fires ONCE a
// decoupled SendMessage("PlayerDied") -- the LOSE a manager (e.g. AutopilotGameOver) reacts
// to ("you ran out of time"). No-op without a receiver. Put it on the GameManager so beating
// it to the exit (gameover.ReachedGoal) wins first. Deterministic: no RNG, no time-of-day; it
// only reads Time.deltaTime.
public class __CLASS__ : MonoBehaviour
{
    ...
    {
        int secs = Mathf.CeilToInt(Mathf.Max(0f, remaining));
        GUI.Label(new Rect(Screen.width - 130f, 10f, 120f, 24f), "Deadline: " + secs);
    }
}
"""
...
_ESCORT_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. An escort VIP -- the thing you protect. It
// walks ITSELF toward a goal object (found by name, decoupled) and on arrival fires a
// one-time SendMessage("ReachedGoal") to the GameManager (the WIN). Tag this object
// "Player" so the enemy AI marches at it (and a goal zone's "Player" check fires the
// same win when it arrives -- two robust paths); pair it with `health` so destroying it
// ends the game (its Die -> "PlayerDied" = LOSE). You drive a separate, untagged Hero.
// Deterministic: it only reads positions (MoveTowards), no RNG, no time-of-day.
public class __CLASS__ : MonoBehaviour
{
    public string goalName = "Goal";
    public float speed = __SPEED__f;
    public float arriveRange = 1.25f;
    public float groundY = 0.5f;
    ...
    void Start()
    {
        GameObject g = GameObject.Find(goalName);
        if (g != null) goal = g.transform;
    }

    void Update()
    {
        if (goal == null || arrived) return;
        Vector3 target = new Vector3(goal.position.x, transform.position.y, goal.position.z);
        if (Vector3.Distance(transform.position, target) <= arriveRange)
        {
            arrived = true;
            GameObject.Find("GameManager")?.SendMessage("ReachedGoal", SendMessageOptions.DontRequireReceiver);
            return;
...
_BOSS_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. A BOSS -- a single high-HP enemy for a duel.
// It chases the Player and melee-attacks on a cooldown (like the basic enemy AI) but has
// a big health pool you must whittle down. Takes damage via SendMessage("TakeDamage"); on
// death it grants xpReward to the Player (SendMessage "AddXP") and DESTROYS itself, so
// AutopilotGameOver's clear-all-enemies WIN fires (tag the boss Enemy). Draws a boss HP
// bar across the top. Decoupled (no hard refs), deterministic (only distance + time, no RNG).
public class __CLASS__ : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUI.Box(new Rect(x, 20f, w, 22f), "BOSS " + currentHP + "/" + maxHP);
        GUI.Box(new Rect(x, 20f, w * frac, 22f), "");
    }
}
"""

_COLLECTRACE_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. A collector-race manager: a countdown you must
// beat by collecting EVERYTHING. Each frame it counts objects named "Collectible_*" (by
// name, decoupled -- no custom tags); when the last one is gone it SendMessages a one-time
// "ReachedGoal" (the WIN, reusing AutopilotGameOver's hook), and if the countdown reaches
// zero first it SendMessages "PlayerDied" (the LOSE -- the studio's first losing deadline).
// Draws "Collected got/total" + the remaining time. Deterministic: only counts +
// Time.deltaTime (no RNG, no time-of-day). Fires each outcome once. Put it on the manager.
public class __CLASS__ : MonoBehaviour
{
    public float duration = 30f;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < all.Length; i++)
        {
            if (all[i].name.StartsWith("Collectible")) count++;
        }
        if (count > total) total = count;        // remember the peak as the target
    ...
    void OnGUI()
    {
        int secs = Mathf.CeilToInt(Mathf.Max(0f, remainingTime));
        GUI.Label(new Rect(Screen.width / 2f - 120f, 10f, 260f, 24f),
                  "Collected: " + collected + "/" + total + "   Time: " + secs);
    }
}
"""

_LOCKGOAL_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. A LOCKED exit -- the win zone of a key-and-door game.
// Each frame it counts objects named "Key_*" (by name, decoupled -- no custom tags); the door
// is UNLOCKED once none remain (you have grabbed them all). When the Player enters the trigger
// AND it is unlocked, it SendMessages a one-time "ReachedGoal" (the WIN, reusing
// AutopilotGameOver's hook); while still locked, entering it does nothing but the HUD shows how
// many keys are left. Draws "Keys left: N" / "Exit OPEN". Deterministic: only counts by name,
// no RNG. Needs a trigger Collider.
[RequireComponent(typeof(Collider))]
public class __CLASS__ : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < all.Length; i++)
        {
            if (all[i].name.StartsWith("Key")) count++;
        }
        keysLeft = count;
    }
    ...
    void OnGUI()
    {
        string msg = keysLeft > 0 ? ("Keys left: " + keysLeft) : "Exit OPEN";
        GUI.Label(new Rect(Screen.width / 2f - 60f, 34f, 160f, 24f), msg);
    }
}
"""

_SAFEZONE_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. A SAFE ZONE -- a circular retreat the chasers cannot
// enter. In LateUpdate (AFTER the enemies' own movement that frame) it finds every object named
// "Enemy_*" (by name, decoupled -- no custom tags) within `radius` and clamps it back out to the
// boundary, so a player standing near the centre is safe from the chase. Purely defensive +
// deterministic: only positions, no RNG. A no-op when there are no chasers. Stand near the middle
// -- enemies pile up at the edge, so the rim is NOT safe.
public class __CLASS__ : MonoBehaviour
{
    public float radius = 3f;
    ...
    void LateUpdate()
    {
        ...
        {
            Transform t = all[i];
            if (!t.name.StartsWith("Enemy")) continue;
            Vector3 d = t.position - c;
            d.y = 0f;
            float dist = d.magnitude;
            if (dist > 0.0001f && dist < radius)
            {
                Vector3 p = c + d.normalized * radius;
                p.y = t.position.y;
                t.position = p;
            }
        }
    ...
    void OnGUI()
    {
        GUI.Label(new Rect(10f, 58f, 240f, 24f), "Safe zone: hide in the middle");
    }
}
"""

_TURRET_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. A stationary TURRET: every `cooldown` seconds it
// finds the Player within `range`, aims at it, and damages it via SendMessage("TakeDamage")
// (decoupled -- no hard reference to a health type; a no-op if the player has none). It does
// NOT move and is NOT tagged Enemy, so you survive it by DODGING to the goal, not by killing
// it (a 'run the gauntlet' threat). Deterministic: only distance + time, no RNG.
public class __CLASS__ : MonoBehaviour
{
...
_HITFLASH_TEMPLATE = """using UnityEngine;

// Auto-generated by UnityTools autopilot. Visual HIT FEEDBACK (juice): when this object is
// damaged (it receives SendMessage("TakeDamage")) its renderer briefly flashes `flashColor`,
// then lerps back to its original color over `flashTime`. Purely cosmetic and decoupled -- it
// runs ALONGSIDE whatever else handles TakeDamage (boss/health/reward). Best on a high-HP
// target (a boss) where the repeated flash is actually visible. Deterministic: only Time, no
// RNG. A no-op on objects without a Renderer.
public class __CLASS__ : MonoBehaviour
{
    ...
    {
        if (rend != null) timer = flashTime;
    }

    void Update()
    {
        if (rend == null || timer <= 0f) return;
        timer -= Time.deltaTime;
        float t = Mathf.Clamp01(timer / flashTime);
        rend.material.color = Color.Lerp(baseColor, flashColor, t);
        if (timer <= 0f) rend.material.color = baseColor;
    }
}
"""
...
_SCRIPT_TEMPLATES: dict[str, tuple[str, str, tuple[float, float, float], float]] = {
    ...
    "wrapmover": ("AutopilotWrapMover", _WRAPMOVER_TEMPLATE, (1.0, 0.0, 0.0), 4.0),
    ...
    "deadline": ("AutopilotDeadline", _DEADLINE_TEMPLATE, (0.0, 0.0, 0.0), 0.0),
    ...
    "escort": ("AutopilotEscort", _ESCORT_TEMPLATE, (0.0, 0.0, 0.0), 2.0),
    "boss": ("AutopilotBoss", _BOSS_TEMPLATE, (0.0, 0.0, 0.0), 0.0),
    "collectrace": ("AutopilotCollectRace", _COLLECTRACE_TEMPLATE, (0.0, 0.0, 0.0), 0.0),
    "lockgoal": ("AutopilotLockGoal", _LOCKGOAL_TEMPLATE, (0.0, 0.0, 0.0), 0.0),
    "safezone": ("AutopilotSafeZone", _SAFEZONE_TEMPLATE, (0.0, 0.0, 0.0), 0.0),
    "turret": ("AutopilotTurret", _TURRET_TEMPLATE, (0.0, 0.0, 0.0), 0.0),
    "hitflash": ("AutopilotHitFlash", _HITFLASH_TEMPLATE, (0.0, 0.0, 0.0), 0.0),
}
