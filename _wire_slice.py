"""Phase 136: recompile (wire_playable_slice goes live) then attach the
Gameplay.* scripts to ForgottenValley_VS objects so the slice is
genuinely playable. Hardened pattern (focus + real main-thread wait +
fresh reconnect). Daemon is paused.
"""
import time, json
...
# QA shot near the hero (close-up so HDRP capture is reliable)
...
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_playable_wired"))[:120])
print("WIRE DONE ok=%s wired=%s missing=%s" % (
