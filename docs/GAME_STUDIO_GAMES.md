...
| `survival` | Ground + WASD player + N elevated hazard **spawners** (raining cubes = chaos/clutter) + N deadly `killzone` floor patches (touch → respawn) + a **goal** exit. Brave the cluttered, hazard-strewn arena and reach the exit to **WIN** (a hidden GameManager lands the win screen). Used to have no threat and no win — now it has both | "sağ kalma oyunu kur" / "build me a survival game" |
...
| `chase` | Ground + player + score HUD + N enemies that **chase** you (follow + killzone) + N collectibles to grab while escaping + a central **safe zone** to retreat into (the new `safezone` clamps the chasers out of its middle) + goal | "kovalamaca oyunu kur" / "build me a chase game" |
...
| `runner` | An **endless runner** (the first non-arena-style type): an **auto-running** player (forward +Z, A/D strafe, Space jump) + a distance **score** it feeds itself + N weaving `killzone` obstacles that snap you back to the start on touch + N **coins** (`collectible`) to grab — each just before an obstacle, on a different lane, so you weave to collect then weave to dodge (coins feed the same score HUD). Endless — no goal/win, get as far + grab as much as you can | "runner oyunu kur" / "endless runner yap" / "koşu oyunu" |
...
| `speedrun` | A **race to the exit** -- the first type that races a DEADLINE TO A DESTINATION: a WASD player crosses from the near end to a `goal` at the far end before a **losing countdown** (the new `deadline`) runs out. N deadly `killzone` hazards in between **respawn** you on contact -- a setback that bleeds the clock. The GameManager runs `deadline` (SendMessages "PlayerDied" -- the **LOSE** -- at zero) + `gameover` (reaching the goal SendMessages "ReachedGoal" -- the **WIN** -- which fires first if you make it) + title + sound. Distinct from `time_survival` (outlast a timer to WIN) and `collector_race` (collect all before the clock) | "speedrun oyunu kur" / "beat the clock" / "süre dolmadan çık" |
| `keydoor` | A **key-and-door** game -- the first type with a FETCH-THEN-EXIT GATE: a WASD player + score grabs all N keys (`collectible`s named `Key_*`), dodging N deadly `killzone` hazards that respawn you, then reaches a **locked exit** at the far end. The exit is the new `lockgoal` (AutopilotLockGoal): it counts the remaining `Key_*` **by name** and stays LOCKED until none are left, then -- when the player reaches it -- SendMessages "ReachedGoal" (the **WIN**, reusing `gameover`'s hook). Distinct from `collectathon` (collect at the goal, no gate) and `collector_race` (a clock): the keys UNLOCK a separate exit you must then reach. No hard lose (a hazard just respawns you) | "keydoor oyunu kur" / "anahtarlı kapı oyunu" / "kilitli çıkış" |
| `frogger` | A **crossing** game -- the first about THREADING LANES OF TRAFFIC: a player at the near edge crosses to a `goal` at the far edge through L **phase-staggered lanes** of one-way traffic. Each lane is a row of cubes carrying the new `wrapmover` (glide +X and wrap around -- an endless stream) + `killzone` (touch -> respawn). The lanes' gaps are offset so they do not line up, so you weave one lane at a time. A reach-the-goal **WIN** (gameover), no combat. Distinct from `dodge` (scattered drifting hazards): here the threats are ORGANISED into crossing lanes | "frogger oyunu kur" / "karşıdan karşıya geç" / "lane crossing" |
...
| `deadline` / `zamanaşımı` / `sonsüre` | AutopilotDeadline | A **losing countdown** -- the mirror of `timer`: counts `duration` seconds down (freezes while paused), draws the remaining time top-right, and on zero fires once a decoupled `SendMessage("PlayerDied")` — the **LOSE** `AutopilotGameOver` reacts to ("you ran out of time"). Put it on the GameManager next to `gameover` so reaching the goal (a WIN) fires first. Deterministic (reads only `Time.deltaTime`) |
...
| `safezone` / `güvenli` / `sığınak` | AutopilotSafeZone | A **safe zone** — a circular retreat the chasers cannot enter: in `LateUpdate` (after the enemies move) it finds every object named `Enemy_*` **by name** within `radius` and clamps it back out to the boundary, so a Player near the middle is safe (the rim is not — enemies pile up there). Purely defensive + deterministic (positions only, no RNG); a no-op without chasers |
...
| `lockgoal` / `kilitli kapı` / `locked exit` | AutopilotLockGoal | A **locked exit** -- the win zone of a key-and-door game: each frame counts objects named `Key_*` **by name** (decoupled, no tags) and is UNLOCKED once none remain; when the Player enters its trigger AND it is unlocked it SendMessages a one-time "ReachedGoal" (**WIN**, reusing `gameover`'s hook), else it stays locked. Draws "Keys left: N" / "Exit OPEN". Deterministic (counts by name, no RNG) |
...
- en: "build me a game", "make a collectathon/dodge/survival/platformer/chase/maze/arena/horde/runner/tower_defense/time_survival/stealth/puzzle/hold/escort/boss/collector_race/twin_stick/speedrun/keydoor/frogger game"
