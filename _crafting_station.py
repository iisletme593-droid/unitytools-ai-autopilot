"""Phase 153: P1 'crafting station'. Recompile (wire_playable_slice
now also creates CraftingBench_Camp + attaches Gameplay.CraftingStation;
InventoryComponent gained CountOf/HasItems/TryConsume/TrySpendGold),
reopen ForgottenValley_VS, wire the slice, confirm it landed +
missing_types empty, screenshot. Resilient-retry absorbs daemon overlap.
"""
import time, json
...
d = rcall("get_object_details", {"name": "CraftingBench_Camp"}, t=20)
comps = d.get("components") if isinstance(d, dict) else None
print("CraftingBench_Camp comps:", json.dumps(comps, ensure_ascii=False)[:200])
...
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_crafting_station"))[:120])
print("CRAFTING-STATION DONE ok=%s wired=%s missing=%s" % (
