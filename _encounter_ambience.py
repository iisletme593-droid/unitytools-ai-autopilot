"""Phase 152: P1 'ambient encounter sesleri'. Recompile
(wire_playable_slice now also attaches Gameplay.EncounterAmbience),
reopen ForgottenValley_VS, wire the slice, confirm it landed +
missing_types empty, screenshot. Resilient-retry pattern absorbs any
brief overlap with the autopilot daemon's ~2s cycle.
"""
import time, json
...
print("WIRE:", json.dumps(res, ensure_ascii=False)[:780])

d = rcall("get_object_details", {"name": "TI_EncounterAmbience"}, t=20)
comps = d.get("components") if isinstance(d, dict) else None
print("TI_EncounterAmbience comps:", json.dumps(comps, ensure_ascii=False)[:200])
...
rcall("set_scene_view", {"pivot_x": hx, "pivot_y": hy + 1.5, "pivot_z": hz,
      "size": 14, "pitch": 6, "yaw": 35}, t=30, retries=2)
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_encounter_ambience"))[:120])
print("ENCOUNTER-AMBIENCE DONE ok=%s wired=%s missing=%s" % (
    isinstance(res, dict) and res.get("ok"),
    res.get("wired_count") if isinstance(res, dict) else "?",
    res.get("missing_types") if isinstance(res, dict) else "?"))
