"""Phase 160 (P2 FINAL): 'dialog and lore pickup'. Recompile
...
for nm in ("LoreNote_BriarCamp", "LoreNote_Path"):
    d = rcall("get_object_details", {"name": nm}, t=20)
    comps = d.get("components") if isinstance(d, dict) else None
    print(nm, "comps:", json.dumps(comps, ensure_ascii=False)[:240])
...
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_lore"))[:120])
print("LORE DONE ok=%s wired=%s missing=%s" % (
