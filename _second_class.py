"""Phase 157 (P2): 'second class prototype'. Recompile
(wire_playable_slice now also attaches Gameplay.ClassPrototype),
...
d = rcall("get_object_details", {"name": "ClassAltar_Skirmisher"}, t=20)
comps = d.get("components") if isinstance(d, dict) else None
print("ClassAltar_Skirmisher comps:", json.dumps(comps, ensure_ascii=False)[:200])
...
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_second_class"))[:120])
print("SECOND-CLASS DONE ok=%s wired=%s missing=%s" % (
