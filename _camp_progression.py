"""Phase 155 (final P1): 'camp progression'. Recompile
(wire_playable_slice now also attaches Gameplay.CampProgression),
...
print("WIRE:", json.dumps(res, ensure_ascii=False)[:820])

d = rcall("get_object_details", {"name": "TI_CampProgression"}, t=20)
comps = d.get("components") if isinstance(d, dict) else None
print("TI_CampProgression comps:", json.dumps(comps, ensure_ascii=False)[:200])
...
rcall("set_scene_view", {"pivot_x": hx, "pivot_y": hy + 1.5, "pivot_z": hz,
      "size": 16, "pitch": 6, "yaw": 35}, t=30, retries=2)
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_camp_progression"))[:120])
print("CAMP-PROGRESSION DONE ok=%s wired=%s missing=%s" % (
