"""Phase 159 (P2): 'elite encounter variations'. Recompile
...
for nm in ("EliteBriarbound_Champion", "Enemy_Briarbound_01"):
    d = rcall("get_object_details", {"name": nm}, t=20)
    comps = d.get("components") if isinstance(d, dict) else None
    print(nm, "comps:", json.dumps(comps, ensure_ascii=False)[:260])
...
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_elite"))[:120])
print("ELITE DONE ok=%s wired=%s missing=%s" % (
