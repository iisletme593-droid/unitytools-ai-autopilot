"""Phase 164 (P3): 'RestTotem respawn anchor + heal rest'.
RestTotem.cs + RestTotemRegistry + GameManager.RespawnPlayer nearest-totem.
Recompile bridge, re-wire, verify RestTotem_Camp (startActive) and
RestTotem_Bridge exist with RestTotem component, screenshot.
"""
import time, json
...
# Verify totems + their components
for name in ["RestTotem_Camp", "RestTotem_Bridge", "SK_Hero"]:
    d = rcall("get_object_details", {"name": name}, t=20)
    comps = d.get("components") if isinstance(d, dict) else []
    has_rt = any("RestTotem" in str(c) for c in (comps or []))
    has_si = any("StatusInfliction" in str(c) for c in (comps or []))
    print(f"  {name}: RestTotem={has_rt} StatusInfliction={has_si} comps={json.dumps(comps)[:90]}")
...
rcall("set_scene_view", {"pivot_x": hx, "pivot_y": hy + 2, "pivot_z": hz,
      "size": 16, "pitch": 8, "yaw": 28}, t=30, retries=2)
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_phase164_rest_totem"))[:120])
...
print("PHASE-164 DONE ok=%s wired=%s missing=%s totems=%s si=%s" % (
    isinstance(res, dict) and res.get("ok"),
    wired, missing, totem_wired, si_wired))
