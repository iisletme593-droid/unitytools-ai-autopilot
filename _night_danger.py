"""Phase 150: P1 'gecenin daha tehlikeli hissedilmesi'. Recompile
(wire_playable_slice now also attaches Gameplay.NightDanger), reopen
ForgottenValley_VS, wire the slice, confirm NightDanger landed +
missing_types empty, screenshot. Daemon is sleeping (~20 min window).
"""
import time, json
...
print("WIRE:", json.dumps(res, ensure_ascii=False)[:700])

nd = rcall("get_object_details", {"name": "TI_NightDanger"}, t=20)
comps = nd.get("components") if isinstance(nd, dict) else None
print("TI_NightDanger comps:", json.dumps(comps, ensure_ascii=False)[:240])
...
time.sleep(1.4)
print("shot:", json.dumps(studio_capture_screenshot(name="fv_night_danger"))[:120])
print("NIGHT-DANGER DONE ok=%s wired=%s missing=%s" % (
